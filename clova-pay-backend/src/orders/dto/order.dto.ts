...
export class CreateOrderDto {
  ...
  @ApiProperty({
    description: 'The Stacks address of the sender',
  ...
  @ApiProperty({
    description: 'The fiat amount in minor units (e.g., kobo, cents)',
    example: 15000000,
  })
  @IsNumber()
  @Min(0)
  ...
  @ApiPropertyOptional({
    description: 'SIP-010 token contract address (if token order)',
    example: 'ST1PQ...token',
  })
  @IsString()
  @IsOptional()
...
export class ProcessOrderDto {
  @ApiProperty({
    enum: ['cUSD_CELO', 'USDC_BASE', 'USDCX_STACKS'],
    description: 'Settlement rail asset',
  })
  @IsString()
